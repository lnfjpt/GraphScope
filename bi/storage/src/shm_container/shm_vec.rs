use crate::shm_container::hack::SafeMutPtr;
use libc::{
    ...
    T: Copy + Sized,
{
    ...
    pub fn create(name: &str, len: usize) -> Self {
        let cstr_name = CString::new(name).unwrap();
        let fd: RawFd =
            unsafe { shm_open(cstr_name.as_ptr(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR) };
        if fd == -1 {
    ...
    T: Copy + Sized + Send + Sync,
{
    ...
    pub fn inplace_parallel_range_move_beta(
        &mut self,
        new_size: usize,
        range_diff: &[(usize, usize, i64)],
    ) {
        if range_diff.len() == 1 {
        ...
        if range_diff.len() < rayon::current_num_threads() {
            ...
            range_diff.par_iter().for_each(|(from, to, diff)| {
                let (new_from, new_to) = if *diff >= 0 {
                    (*from + *diff as usize, *to + *diff as usize)
                } else {
                    (*from - (diff.abs() as usize), *to - (diff.abs() as usize))
    ...
    pub fn inplace_parallel_range_move(
        &mut self,
        new_size: usize,
        range_diff: &[(usize, usize, i64)],
    ) {
        ...
        if range_diff.len() < rayon::current_num_threads() {
            ...
            range_diff[1..].par_iter().for_each(|(from, to, diff)| {
                let (new_from, new_to) = if *diff >= 0 {
                    (*from + *diff as usize, *to + *diff as usize)
                } else {
                    (*from - (diff.abs() as usize), *to - (diff.abs() as usize))
...
impl SharedStringVec {
    pub fn load(path: &str, name: &str) {
        SharedVec::<usize>::load(
            format!("{}_offset", path).as_str(),
            format!("{}_offset", name).as_str(),
        );
        SharedVec::<u16>::load(
            format!("{}_length", path).as_str(),
            format!("{}_length", name).as_str(),
        );
        SharedVec::<u8>::load(
            format!("{}_content", path).as_str(),
            format!("{}_content", name).as_str(),
        );
    }
