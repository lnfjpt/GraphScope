...
use crate::mmap_container::{MmapStringVec, MmapVec};
use crate::shm_container::{SharedStringVec, SharedVec};
...
impl Int32Column {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<i32>::create(name, len),
        }
    }
...
impl UInt32Column {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<u32>::create(name, len),
        }
    }
...
impl Int64Column {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<i64>::create(name, len),
        }
    }
...
impl UInt64Column {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<u64>::create(name, len),
        }
    }
...
impl IDColumn {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<DefaultId>::create(name, len),
        }
    }
    ...
    pub fn open(path: &str) -> Self {
        Self {
            data: SharedVec::<DefaultId>::open(path),
        }
    }
}
...
impl Column for IDColumn {
    ...
    fn set_column_batch(&mut self, index: &Vec<usize>, col: &Box<dyn HeapColumn>) {
        if col.as_any().is::<IDHColumn>() {
            let casted_col = col.as_any().downcast_ref::<IDHColumn>().unwrap();
            let mut_self_data = SafeMutPtr::new(&mut self.data);
            index.par_iter().enumerate().for_each(|(idx, val)| {
                if *val != usize::MAX {
                    assert!(*val < self.data.len());
...
impl DoubleColumn {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<f64>::create(name, len),
        }
    }
...
impl StringColumn {
    ...
    pub fn open(path: &str) -> Self {
        Self {
            data: SharedStringVec::open(path),
        }
    }
}
...
impl Column for LowUsageStringColumn {
    ...
    fn get(&self, index: usize) -> Option<RefItem> {
        if index < self.data.len() {
            ...
            self.append_data
                .get(index - self.data.len())
                .map(|x| RefItem::String(x))
        } else {
            None
        }
    }
...
impl LCStringColumn {
    pub fn load(path: &str, name: &str) {
        SharedVec::<u16>::load(
            format!("{}_index", path).as_str(),
            format!("{}_index", name).as_str(),
        );
        SharedStringVec::load(
            format!("{}_data", path).as_str(),
            format!("{}_data", name).as_str(),
        );
    }
...
impl Column for LCStringColumn {
    ...
    fn get(&self, index: usize) -> Option<RefItem> {
        if index < self.index.len() {
            Some(RefItem::String(
                self.data.get_unchecked(self.index[index] as usize),
            ))
        } else {
            None
        }
...
impl LowUsageLCStringColumn {
    ...
    pub fn open(mmap_path: &str, shm_path: &str) -> Self {
        let data = MmapStringVec::open(format!("{}_data", mmap_path).as_str())
            .expect("Failed to open data of LowUsageLCStringColumn");
        let mut table = HashMap::new();
        let len = data.len();
        for i in 0..len {
            table.insert(data.get_unchecked(i).to_string(), i as u16);
        }
        let index = MmapVec::<u16>::open(format!("{}_index", mmap_path).as_str())
            .expect("Failed to open index of LowUsageLCStringColumn");
...
impl Column for LowUsageLCStringColumn {
    ...
    fn get(&self, index: usize) -> Option<RefItem> {
        if index < self.index.len() {
            ...
            Some(RefItem::String(self.data.get_unchecked(
                self.append_index[index - self.index.len()] as usize,
            )))
        } else {
            None
        }
...
impl DateColumn {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<Date>::create(name, len),
        }
    }
...
impl Column for DateColumn {
    ...
    fn set_column_batch(&mut self, index: &Vec<usize>, col: &Box<dyn HeapColumn>) {
        if col.as_any().is::<DateHColumn>() {
            let casted_col = col.as_any().downcast_ref::<DateHColumn>().unwrap();
            let mut_self_data = SafeMutPtr::new(&mut self.data);
            index.par_iter().enumerate().for_each(|(idx, val)| {
                if *val < usize::MAX {
                    assert!(*val < self.data.len());
...
impl DateTimeColumn {
    pub fn create(name: &str, len: usize) -> Self {
        Self {
            data: SharedVec::<DateTime>::create(name, len),
        }
    }
    ...
    pub fn open(path: &str) -> Self {
        Self {
            data: SharedVec::<DateTime>::open(path),
        }
    }
}
...
impl Column for DateTimeColumn {
    ...
    fn set_column_batch(&mut self, index: &Vec<usize>, col: &Box<dyn HeapColumn>) {
        if col.as_any().is::<DateTimeHColumn>() {
            let casted_col = col.as_any().downcast_ref::<DateTimeHColumn>().unwrap();
            let mut_self_data = SafeMutPtr::new(&mut self.data);
            index.par_iter().enumerate().for_each(|(idx, val)| {
                if *val != usize::MAX {
