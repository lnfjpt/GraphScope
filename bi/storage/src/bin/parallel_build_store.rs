...
use clap::{App, Arg};
use env_logger;
use storage::graph_loader_beta::{load_graph, load_graph_no_corner};
use storage::schema::{CsrGraphSchema, InputSchema};
use storage::shuffler::Shuffler;
use storage::types::*;
use storage::vertex_loader::load_vertex;
...
fn main() {
    ...
    let input_schema_file = matches.value_of("input_schema_file").unwrap().to_string();
    let graph_schema_file = matches.value_of("graph_schema_file").unwrap().to_string();
    ...
    let reader_num = matches
        .value_of("read_concurrency")
        .unwrap()
        .parse::<usize>()
        .expect("read concurrency should be a number");
    let offset = matches
        .value_of("offset")
        .unwrap()
        .parse::<usize>()
