    ...
    pub fn load(prefix: &str, partition: usize, shm_name: &str, mmap_prefix: Option<&str>) {
        ...
        for i in 0..vertex_label_num {
            Table::load(
                ...
                graph_schema.get_vertex_header(i as LabelId).unwrap(),
            ...
            for edge_label in 0..edge_label_num {
                for dst_label in 0..vertex_label_num {
                    if let Some(_) = graph_schema.get_edge_header(
                        ...
                        let load_strategy = graph_schema.get_edge_load_strategy(
                            src_label as LabelId,
                            edge_label as LabelId,
                            dst_label as LabelId,
                        );
                        if load_strategy == LoadStrategy::OnlyOut
                            || load_strategy == LoadStrategy::BothOutIn
                        {
                            ...
                            let oe_name_prefix = format!(
                                "{}_oe_{}_{}_{}",
                                shm_name, src_label, edge_label, dst_label
                            );
                        ...
                        if load_strategy == LoadStrategy::OnlyIn
                            || load_strategy == LoadStrategy::BothOutIn
                        {
                        ...
                        if let Some(header) = graph_schema.get_edge_header(
                            ...
                            if !header.is_empty() {
                                oe_edge_prop_table.insert(
                                    index,
                                    Table::open(oep_prefix.as_str(), header, None, shm_name),
                                );
                            }
                        }
                        ...
                        if let Some(header) = graph_schema.get_edge_header(
                            ...
                            if !header.is_empty() {
                                ie_edge_prop_table.insert(
                                    index,
                                    Table::open(iep_prefix.as_str(), header, None, shm_name),
                                );
                            }
                        }
                    }
        ...
        for (&vertex_label, vertex_set) in self.pending_to_delete.iter() {
            ...
            for e_label_i in 0..self.edge_label_num {
                for src_label_i in 0..self.vertex_label_num {
                    ...
                    let load_strategy = self.graph_schema.get_edge_load_strategy(
                        src_label_i as LabelId,
                        e_label_i as LabelId,
                        vertex_label as LabelId,
                    );
                    if load_strategy == LoadStrategy::BothOutIn
