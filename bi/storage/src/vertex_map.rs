    ...
    I: Send + Sync + IndexType,
{
    pub fn load(prefix: &str, num_labels: usize, name: &str) {
        let mut vertices_num =
            SharedVec::<usize>::create(format!("{}_vm_vnum", name).as_str(), num_labels);
        for i in 0..num_labels {
            ...
            vm_tomb.as_mut_slice().par_iter_mut().for_each(|x| {
                *x = 0_u8;
            });
    ...
    pub fn open(prefix: &str, num_labels: usize) -> Self {
        ...
        let mut tombs = vec![];
        for i in 0..num_labels {
            tombs.push(SharedVec::<u8>::open(
                format!("{}_vm_tomb_{}", prefix, i as usize).as_str(),
            ));
        }
        Self {
