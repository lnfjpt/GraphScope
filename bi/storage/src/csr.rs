...
fn generate_new_offsets<I: IndexType>(
    vertex_num: usize,
    old_degree: &[i32],
    offsets: &mut SharedVec<usize>,
    parsed_vertices: &Vec<I>,
    ...
    let mut diff_list: Vec<(usize, i32)> = diff_table.par_iter().map(|(x, y)| (*x, *y)).collect();
    ...
    let e_range_diff: Vec<(usize, usize, i64)> = v_range_diff
        ...
        .map(|(v_begin, v_end, diff)| {
            (
                offsets[*v_begin],
                offsets[*v_end - 1] + old_degree[*v_end - 1] as usize,
                *diff,
            )
        })
        .collect();
...
impl<G: IndexType, I: IndexType> Csr<G, I> {
    pub fn load(prefix: &str, name: &str) {
        SharedVec::<usize>::load(
            format!("{}_meta", prefix).as_str(),
            format!("{}_meta", name).as_str(),
        );
        SharedVec::<G>::load(
            format!("{}_nbrs", prefix).as_str(),
            format!("{}_nbrs", name).as_str(),
        );
        SharedVec::<usize>::load(
        ...
        SharedVec::<i32>::load(
            format!("{}_degree", prefix).as_str(),
            format!("{}_degree", name).as_str(),
        );
    }
...
impl<G: IndexType, I: IndexType> CsrTrait<G, I> for Csr<G, I> {
    ...
    fn insert_edges_beta(
        ...
        vertex_num: usize,
        edges: &Vec<(G, G)>,
        insert_edges_prop: Option<&DataFrame>,
        reverse: bool,
        ...
        let (e_range_diff, new_edges_num) = generate_new_offsets(
            vertex_num,
