...
pub(crate) fn get_entry_partition(entry: &DynEntry, workers: usize, num_servers: usize) -> u64 {
    let v = entry.as_vertex().unwrap();
    get_partition(&(v.id() as u64), workers as usize, num_servers)
}
