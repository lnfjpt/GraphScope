...
//
// /// this is the handwritten + record + alias_opt version of partial bi2, with only two-hops expxansion by tag_class -- tag -- message;
// /// specifically, alias_opt means that we do not append with alias when fist meet the entry (e.g., tag_class);
// /// instead, we append "parent" with the alias when a new_entry is generated.
// pub fn bi2_hop_record_aliasopt(conf: JobConf, date: String, tag_class: String) -> ResultStream<u64> {
...
//                             new_record.append(tag_runtime_vertex, None);
...
//                     Ok(get_record_curr_partition(tag_record, workers as usize, pegasus::get_servers_len()))
//                 })
...
//                         new_tag_message_record.append_with_head_alias(vertex, Some(0));
