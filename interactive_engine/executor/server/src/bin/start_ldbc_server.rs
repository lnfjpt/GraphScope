...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    GraphDB::<usize, usize>::load(graph_data_str, config.partition_id, name, None);
    let shared_graph =
        Arc::new(RwLock::new(GraphDB::<usize, usize>::open(graph_data_str, None, config.partition_id)));
